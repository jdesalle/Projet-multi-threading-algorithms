...
void *producer(void* args){
    ...
    while(n_prod < 1024 - NPROD + 2){
        my_sem_wait(&empty); //producer must wait until one slot is empty
        lock(&mutex); //lock buffer
        buffer[last] = nb_rand();
        //printf("The item %d has been produced and placed at the position %d\n", buffer[last], last);
        last = (last+1)%slots; //we move to the next slot
        //count_p++;
        //printf("Count_p = %d\n", count_p);
        unlock(&mutex); //unlock buffer
        //printf("mutex unlocked\n");
        my_sem_post(&full); //wakeup the consumer
...
void *consumer(void* args){
    ...
    while(n_cons < 1024 - NCONS + 2){
        my_sem_wait(&full); //consumer must wait until producer added something
        lock(&mutex);
        ...
        unlock(&mutex);
        // printf("mutex unlocked\n");
        my_sem_post(&empty); //wakeup the producer
